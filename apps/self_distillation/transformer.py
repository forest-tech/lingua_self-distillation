...
class LMTransformer(BaseTransformer):
    ...
    def forward(
        ...
        # Collect hidden states from BaseTransformer layers if requested.
        hidden_states = []
        if return_hidden_states:
            # include embedding output as layer 0
            hidden_states.append(h.clone())

        # If BaseTransformer.forward supports returning per-layer states, use it.
        # Otherwise we perform sequential layer calls to capture states.
        # BaseTransformer.forward(h, ...) returns the final hidden states.
        # We iterate through self.layers to collect intermediate states.
        freq_cis = getattr(self, "rope_embeddings", None)
        if hasattr(self, "layers"):
            # use tok_idx if available
            for layer in self.layers:
                # Some layers accept (h, freq_cis, tok_idx=..., mask=..., attn_impl=...)
                try:
                    h = layer(h, freq_cis, tok_idx=tok_idx, mask=mask, attn_impl=attn_impl)
                except TypeError:
                    # Fall back to calling without freq_cis
                    h = layer(h, tok_idx=tok_idx, mask=mask, attn_impl=attn_impl)
                if return_hidden_states:
                    hidden_states.append(h.clone())
        else:
        ...
        if return_hidden_states:
            # compute per-layer logits by passing each hidden state through LM head
            per_layer_logits = []
            for hs in hidden_states:
                per_layer_logits.append(self.output(self.norm(hs)))

            outputs = {"logits": logits, "hidden_states": hidden_states, "per_layer_logits": per_layer_logits}
            if target is not None:
                outputs["loss"] = cross_entropy(logits, target)
            return outputs
        else:
            if target is not None:
